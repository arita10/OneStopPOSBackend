...
router.post('/', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      const finalItemCost = item.cost || itemCost || 0;
      const quantity = item.quantity || 1;
      const itemPrice = item.price || item.unit_price || 0;
      const itemSubtotal = item.subtotal || (itemPrice * quantity);

      totalTransactionCost += (finalItemCost * quantity);
    ...
    // Profit = Total Revenue (sum of item subtotals) - Total Cost (sum of cost * quantity)
    const calculatedProfit = totalRevenue - totalTransactionCost;
    
    await client.query(
      'UPDATE transactions SET total_profit = $1 WHERE id = $2',
      [calculatedProfit, transaction.id]
    );
